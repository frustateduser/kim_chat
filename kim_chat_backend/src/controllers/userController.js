...
const fetchUserInteractions = async (req, res) => {
    try {
        ...
        const userChats = await Chats.findOne({ userId }).populate('interactions.interactedUserId', 'name username userId');

        if (!userChats) {
            return res.status(204).json({ message: 'No chat interactions found for this user' });
        }
